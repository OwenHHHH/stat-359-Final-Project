# exact copy of generator.py except for changes to logic to ensure that depth
# is always max_depth (not mirrored for additional difficulty)
import random
...
class ExpressionGenerator:
    ...
    def generate(self, current_depth=0, rand_req = 0):
        # If we reached max depth or a random chance, return a number
        if current_depth >= self.max_depth or random.random() < rand_req:
            return str(random.randint(*self.num_range))
        
        # otherwise determine which tree to extend to max_depth
        # to not force a tree to max_depth, return rand_req to 0.3
        strict_options = ['both', 'left', 'right']
        strict_choice = random.choice(strict_options)
        if strict_choice == 'both':
          left = self.generate(current_depth + 1)
          right = self.generate(current_depth + 1)
        elif strict_choice == 'left':
          left = self.generate(current_depth + 1)
          right = self.generate(current_depth + 1, rand_req = 0.3)
        else:
          left = self.generate(current_depth + 1, rand_req = 0.3)
          right = self.generate(current_depth + 1)
        # define operation
        op = random.choice(['+', '-'])
...
if __name__ == "__main__":
    # Usage
    for _ in range(5):
        generator = ExpressionGenerator(max_depth=3, invalid_rate=0)
        new_expr = generator.generate()

        print(f"Generated Expression: {new_expr}")

    for _ in range(5):
        generator = ExpressionGenerator(max_depth=4, invalid_rate=0)
        new_expr = generator.generate()
