...
copied from evaluator.py - changed to use strict_depth_generator.py
"""
...
class ModelEvaluator:
    ...
    def _save_results(
        ...
        # Save metrics
        metrics_path = os.path.join(output_dir, f'evaluation_metrics_depth_{self.max_depth}_{timestamp}.json')
        with open(metrics_path, 'w') as f:
            json.dump(metrics, f, indent=2)
        
        # Save sample outputs
        samples_path = os.path.join(output_dir, f'sample_outputs_depth_{self.max_depth}_{timestamp}.json')
        with open(samples_path, 'w') as f:
            json.dump(sample_outputs, f, indent=2)
        
        # Save summary text file
        summary_path = os.path.join(output_dir, f'evaluation_summary_depth_{self.max_depth}_{timestamp}.txt')
